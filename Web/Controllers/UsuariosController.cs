...
namespace PayTollCardApi.Web.Controllers
{
    ...
    public class UsuariosController : ControllerBase
    {
        ...
        public async Task<IActionResult> ObtenerCategoriaPorCedula(string cedula)
        {
            ...
            if (vehiculo == null)
            {
                return NotFound(new { Message = "Vehículo no encontrado para la tarjeta." });
            }

            var categoria = await _context.CategoriasVehiculos.FirstOrDefaultAsync(c => c.IdCategoria == vehiculo.CategoriaVehiculo);
            if (categoria == null)
            {
